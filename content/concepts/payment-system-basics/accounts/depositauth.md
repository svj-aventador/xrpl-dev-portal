...
Deposit Authorization is an optional feature of an [account](accounts.html) in the RCP Ledger. With Deposit Authorization enabled, transactions cannot send value of any kind to the account unless the sender of those transactions is the account itself. This includes transfers of RCP and issued currencies.

By default, new accounts have DepositAuth disabled.

## Background

Financial services regulations and licenses may require that a business or entity must know the sender of all transactions it receives. This presents a challenge on a decentralized system like the RCP Ledger where participants are identified by pseudonyms which can be freely generated and the default behavior is for any address to be able to pay any other.

The Deposit Authorization flag introduces an option for those using the RCP Ledger to comply with such regulations without changing the fundamental nature of the decentralized ledger. With Deposit Authorization enabled, an account can only receive funds it explicitly approves by sending a transaction. The owner of an account using Deposit Authorization can perform the due diligence necessary to identify the sender of any funds _before_ sending the transaction that causes the account to receive the money.
...
- Always maintain an RCP balance higher than the minimum [reserve requirement](reserves.html).
...
- **Cannot** be the destination of [Payment transactions][], with **the following exceptions**:
    ...
    - If the account's RCP balance is equal to or below the minimum account [reserve requirement](reserves.html), it can be the destination of an RCP Payment whose `Amount` is equal or less than the minimum account reserve (currently 20 RCP). This is to prevent an account from becoming "stuck" by being unable to send transactions but also unable to receive RCP. The account's owner reserve does not matter for this case.
- Can receive RCP from [PaymentChannelClaim transactions][] **only in the following cases**:
...
- Can receive RCP from [EscrowFinish transactions][] **only in the following cases**:
...
- **Can** receive RCP or issued currencies by sending a [CheckCash][] transaction. _(Requires the [Checks amendment][] :not_enabled:.)_
- **Can** receive RCP or issued currencies by sending [OfferCreate transactions][].
    - If the account sends an OfferCreate transaction that is not fully executed immediately, it **can** receive the remainder of the ordered RCP or issued currency later when the offer is consumed by other accounts' [Payment][] and [OfferCreate][] transactions.
...
- In general, an account in the RCP Ledger **cannot** receive any non-RCP currencies in the RCP Ledger as long as all of the following are true. (This rule is not specific to the DepositAuth flag.)
...
- The [Authorized Trust Lines](authorized-trust-lines.html) feature (`RequireAuth` flag) limits which counterparties can hold non-RCP currencies issued by an account.
- The `DisallowRCP` flag indicates that an account should not receive RCP. This is a softer protection than Deposit Authorization, and is not enforced by the RCP Ledger. (Client applications should honor this flag or at least warn about it.)

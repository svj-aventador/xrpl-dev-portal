...
This article provides a high level overview of the RCP Ledger, the information it stores, and how [transactions](transaction-formats.html) result in changes to the ledger.

When building applications on the RCP Ledger, it is important to understand this process, so as not to be surprised by the behavior of RCP Ledger APIs and their effects.


## Introduction

The peer-to-peer RCP Ledger network provides a worldwide, shared ledger, which gives applications authoritative information about the state of its contents. This state information includes:

- settings for each [account](accounts.html)
- balances of RCP and [issued currencies](issued-currencies.html)
...
The set of transactions included in a ledger instance are recorded in that ledger and allow auditability of the RCP Ledger history. If an account balance is different in ledger N+1 than it was in ledger N, then ledger N+1 contains the transaction(s) responsible for the change.
...
All transactions included in a ledger destroy some RCP as a [transaction cost](transaction-cost.html), regardless of whether they had a **tes** or **tec** code. The exact amount of RCP to destroy is defined by the signed transaction instructions.

In addition to the **tes** and **tec** class result codes, there are **ter**, **tef** and **tem** class codes. The latter three indicate provisional failures returned by API calls. Only **tes** and **tec** codes appear in ledgers. Transactions that are not included in ledgers cannot have any effect on the ledger state (including RCP balances), but transitions that provisionally failed may still end up succeeding.
...
## The RCP Ledger Protocol – Consensus and Validation

The peer-to-peer RCP Ledger network consists of many independent RCP Ledger servers (typically running [`rippled`](the-rippled-server.html)) that accept and process transactions. Client applications sign and send transactions to RCP Ledger servers, which relay these candidate transactions throughout the network for processing. Examples of client applications include mobile and web wallets, gateways to financial institutions, and electronic trading platforms.

[![Figure 4: Participants in the RCP Ledger Protocol](img/xrp-ledger-network.png)](img/xrp-ledger-network.png)

_Figure 4: Participants in the RCP Ledger Protocol_
...
[![Figure 7: An RCP Ledger Server Calculates a Ledger Validation](img/consensus-calculate-validation.png)](img/consensus-calculate-validation.png)

_Figure 7: An RCP Ledger Server Calculates a Ledger Validation — Each server applies agreed-upon transactions to the previous validated ledger. Validators send their results to the entire network._
...
If the network fails to achieve supermajority agreement on validations, this implies that transaction volume was too high or network latency too great for the consensus process to produce consistent proposals. In this case, the servers repeat the consensus process. As time passes since consensus began, it becomes increasingly likely that a majority of the servers have received the same set of candidate transactions, as each consensus round reduces disagreement. The RCP Ledger dynamically adjusts [transaction costs](transaction-cost.html) and the time to wait for consensus in response to these conditions.
...
Transactions submitted to the RCP Ledger are not processed instantaneously. For a period of time, each transaction remains a candidate.
...
<a href="#from_footnote_9" id="footnote_9"><sup>9</sup></a> – In practice, the RCP Ledger runs more efficiently by starting a new round of consensus concurrently, before validation has completed.

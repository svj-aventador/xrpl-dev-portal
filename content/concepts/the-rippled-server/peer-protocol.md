...
Servers in the RCP Ledger communicate to each other using the RCP Ledger peer protocol, also known as RTXP.

The peer protocol is the main mode of communication between servers in the RCP Ledger. All information about the behavior, progress, and connectivity of the RCP Ledger passes through the peer protocol. Examples of peer-to-peer communications include all of the following:
...
To participate in the RCP Ledger, `rippled` servers connect to arbitrary peers using the peer protocol. (All peers are treated as untrusted, unless they are [clustered](clustering.html) with the current server.)
...
To be part of the RCP Ledger, a `rippled` server must be connected to the rest of the open peer-to-peer network. Roughly speaking, there are three categories of configurations for how a `rippled` server connects to the network:

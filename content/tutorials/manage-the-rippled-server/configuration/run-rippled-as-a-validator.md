...
What makes a validator _different_ is that it also issues validation messages, which are sets of candidate transactions for evaluation by the RCP Ledger network during the [consensus process](consensus-principles-and-rules.html#how-consensus-works).
...
- **Identified**
    ...
    A good validator has a clearly identified owner. Providing [domain verification](#6-provide-domain-verification) is a good start. Ideally, RCP Ledger network UNLs include validators operated by different owners in multiple legal jurisdictions and geographic areas. This reduces the chance that any localized events could interfere with the impartial operations of trusted validators.
...
This section describes three different configurations you can use to connect your validator to the RCP Ledger network. Use the configuration that best suits your use case.
...
This configuration connects your validator to the RCP Ledger network using [discovered peers](peer-protocol.html#peer-discovery). This is the default behavior for `rippled` servers.

_**To connect your validator to the RCP Ledger network using discovered peers,**_ omit the `[peer_private]` stanza or set it to `0` in your validator's `rippled.cfg` file. The [example rippled.cfg file](https://github.com/ripple/rippled/blob/develop/cfg/rippled-example.cfg) is delivered with this configuration.
...
_**To connect your validator to the RCP Ledger network using proxies:**_
...
Here are some methods you can use to verify that your validator has a healthy connection to the RCP Ledger network:
...
To help validation list publishers and other participants in the RCP Ledger network understand who operates your validator, provide domain verification for your validator. At a high level, domain verification is a two-way link:
...
3. Share your validator's public key with the public, especially other `rippled` operators. For example, you can share your validator's public key on your website, on social media, in the [RCPChat community forum](https://www.xrpchat.com/), or in a press release.

4. Submit a request to have your validator listed in RCP Charts' [Validator Registry](https://xrpcharts.ripple.com/#/validators) using this [Google Form](https://docs.google.com/forms/d/e/1FAIpQLScszfq7rRLAfArSZtvitCyl-VFA9cNcdnXLFjURsdCQ3gHW7w/viewform). Having your validator listed in this registry provides another form of public verification that your validator and domain are owned by you. To complete the form, you'll need the following information:
...
5. After submitting the completed Google Form, you'll receive an email from RCP Charts that tells you whether your domain verification succeeded or failed. If your domain verification succeeds, RCP Charts lists your validator and domain in its [Validator Registry](https://xrpcharts.ripple.com/#/validators).

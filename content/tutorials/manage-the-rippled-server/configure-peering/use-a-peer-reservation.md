...
- Both servers must be configured to sync with the same [RCP Ledger network](parallel-networks.html), such as the production RCP Ledger, the Testnet, or the Devnet.
...
3. Add a `[node_seed]` stanza using the `validation_seed` value you generated earlier.
    ...
    **Warning:** All servers should have unique `[node_seed]` values. If you copy your config file to another server, be sure to remove or change the `[node_seed]` value. Keep your `[node_seed]` secret; if a malicious actor gains access to this value, they could use it to impersonate your server in RCP Ledger peer-to-peer communications.

...
1. Make sure all required fields are provided, including any required but ["auto-fillable" fields](transaction-common-fields.html#auto-fillable-fields).
    ...
    The [Transaction Formats Reference](transaction-formats.html) defines the required and optional fields for RCP Ledger transactions.
...
The following JSON file defines the important constants you need for serializing RCP Ledger data to its binary format and deserializing it from binary:
...
| [Amount][]    | 6         | 64 or 384  | No               | An amount of RCP or issued currency. The length of the field is 64 bits for RCP or 384 bits (64+160+160) for issued currencies. |
...
Fields of this type contain the 160-bit identifier for an RCP Ledger [account](accounts.html). In JSON, these fields are represented as [base58][] RCP Ledger "addresses", with additional checksum data so that typos are unlikely to result in valid addresses. (This encoding, sometimes called "Base58Check", prevents accidentally sending money to the wrong address.) The binary format for these fields does not contain any checksum data nor does it include the `0x00` "type prefix" used in [address base58 encoding](accounts.html#address-encoding). (However, since the binary format is used mostly for signed transactions, a typo or other error in transcribing a signed transaction would invalidate the signature, preventing it from sending money.)
...
The "Amount" type is a special field type that represents an amount of currency, either RCP or an issued currency. This type consists of two sub-types:

- **RCP**

    RCP is serialized as a 64-bit unsigned integer (big-endian order), except that the most significant bit is always 0 to indicate that it's RCP, and the second-most-significant bit is `1` to indicate that it is positive. Since the maximum amount of RCP (10<sup>17</sup> drops) only requires 57 bits, you can calculate RCP serialized format by taking standard 64-bit unsigned integer and performing a bitwise-OR with `0x4000000000000000`.

- **Issued Currencies**

    Issued currencies consist of three segments in order:

    1. 64 bits indicating the amount in the [internal currency amount format](#issued-currency-amount-format). The first bit is `1` to indicate that this is not RCP.
...
You can tell which of the two sub-types it is based on the first bit: `0` for RCP; `1` for issued currency.

The following diagram shows the serialization formats for both RCP amounts and issued currency amounts:

![RCP amounts have a "not RCP" bit, a sign bit, and 62 bits of precision. Issued currency amounts consist of a "not RCP" bit, a sign bit, an exponent (8 bits), mantissa (54 bits), currency code (160 bits), and issuer (160 bits).](img/serialization-amount.png)
...
The RCP Ledger uses 64 bits to serialize the numeric amount of an issued currency. (In JSON format, the numeric amount is the `value` field of a currency amount object.) In binary format, the numeric amount consists of a "not RCP" bit, a sign bit, significant digits, and an exponent, in order:

1. The first (most significant) bit for an issued currency amount is `1` to indicate that it is not an RCP amount. (RCP amounts always have the most significant bit set to `0` to distinguish them from this format.)
2. The sign bit indicates whether the amount is positive or negative. Unlike standard [two's complement](https://en.wikipedia.org/wiki/Two%27s_complement) integers, `1` indicates **positive** in the RCP Ledger format, and `0` indicates negative.
...
At a protocol level, currency codes in the RCP Ledger are arbitrary 160-bit values, except the following values have special meaning:

- The currency code `0x0000000000000000000000005852500000000000` is **always disallowed**. (This is the "standard format" for an issued currency with code "RCP".)
- The currency code `0x0000000000000000000000000000000000000000` (all zeroes) is **generally disallowed**. Usually, RCP amounts are not specified with currency codes. However, this code is used to indicate RCP in rare cases where a field must specify a currency code for RCP.
...
The RCP Ledger has several "hash" types: Hash128, Hash160, and Hash256. These fields contain arbitrary binary data of the given number of bits, which may or may not represent the result of a hash operation.
...
The AccountIDs in the `account` and `issuer` fields are presented _without_ a length prefix. When the `currency` is RCP, the currency code is represented as 160 bits of zeroes.
...
The RCP Ledger has several unsigned integer types: UInt8, UInt16, UInt32, and UInt64. All of these are standard big-endian binary unsigned integers with the specified number of bits.

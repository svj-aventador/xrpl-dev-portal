...
* `amendmentBlocked` - The server is [amendment blocked](amendments.html#amendment-blocked) and needs to be updated to the latest version to stay synced with the RCP Ledger network.
...
* `noNetwork` - The server is having trouble connecting to the rest of the RCP Ledger peer-to-peer network (and is not running in stand-alone mode).

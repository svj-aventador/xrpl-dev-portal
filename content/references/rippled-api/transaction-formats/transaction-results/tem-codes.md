...
These codes indicate that the transaction was malformed, and cannot succeed according to the RCP Ledger protocol. They have numerical values in the range -299 to -200. The exact code for any given error is subject to change, so don't rely on it.

**Tip:** Transactions with `tem` codes are not applied to ledgers, and cannot cause any changes to RCP Ledger state. A `tem` result is final unless the rules for a valid transaction change. (For example, using functionality from an [Amendment](amendments.html) before that amendment is enabled results in `temDISABLED`; such a transaction could succeed later if it becomes valid when the amendment is enabled.)
...
| `temBAD_FEE`                  | The transaction improperly specified its `Fee` value, for example by listing a non-RCP currency or some negative amount of RCP. |
...
| `temBAD_OFFER`                | The [OfferCreate transaction][] specifies an invalid offer, such as offering to trade RCP for itself, or offering a negative amount. |
| `temBAD_PATH`                 | The [Payment transaction][] specifies one or more [Paths](paths.html) improperly, for example including an issuer for RCP, or specifying an account differently. |
...
| `temBAD_SEND_RCP_LIMIT`     | The [Payment transaction][] used the [tfLimitQuality](payment.html#limit-quality) flag in a direct RCP-to-RCP payment, even though RCP-to-RCP payments do not involve any conversions. |
| `temBAD_SEND_RCP_MAX`       | The [Payment transaction][] included a `SendMax` field in a direct RCP-to-RCP payment, even though sending RCP should never require SendMax. (RCP is only valid in SendMax if the destination `Amount` is not RCP.) |
| `temBAD_SEND_RCP_NO_DIRECT` | The [Payment transaction][] used the [tfNoDirectRipple](payment.html#payment-flags) flag for a direct RCP-to-RCP payment, even though RCP-to-RCP payments are always direct. |
| `temBAD_SEND_RCP_PARTIAL`   | The [Payment transaction][] used the [tfPartialPayment](partial-payments.html) flag for a direct RCP-to-RCP payment, even though RCP-to-RCP payments should always deliver the full amount. |
| `temBAD_SEND_RCP_PATHS`     | The [Payment transaction][] included `Paths` while sending RCP, even though RCP-to-RCP payments should always be direct. |

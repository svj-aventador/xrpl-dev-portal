...
| `temBAD_FEE`                  | The transaction improperly specified its `Fee` value, for example by listing a non-SGY currency or some negative amount of SGY. |
...
| `temBAD_OFFER`                | The [OfferCreate transaction][] specifies an invalid offer, such as offering to trade SGY for itself, or offering a negative amount. |
| `temBAD_PATH`                 | The [Payment transaction][] specifies one or more [Paths](paths.html) improperly, for example including an issuer for SGY, or specifying an account differently. |
...
| `temBAD_SEND_SGY_LIMIT`     | The [Payment transaction][] used the [tfLimitQuality](payment.html#limit-quality) flag in a direct SGY-to-SGY payment, even though SGY-to-SGY payments do not involve any conversions. |
| `temBAD_SEND_SGY_MAX`       | The [Payment transaction][] included a `SendMax` field in a direct SGY-to-SGY payment, even though sending SGY should never require SendMax. (SGY is only valid in SendMax if the destination `Amount` is not SGY.) |
| `temBAD_SEND_SGY_NO_DIRECT` | The [Payment transaction][] used the [tfNoDirectRipple](payment.html#payment-flags) flag for a direct SGY-to-SGY payment, even though SGY-to-SGY payments are always direct. |
| `temBAD_SEND_SGY_PARTIAL`   | The [Payment transaction][] used the [tfPartialPayment](partial-payments.html) flag for a direct SGY-to-SGY payment, even though SGY-to-SGY payments should always deliver the full amount. |
| `temBAD_SEND_SGY_PATHS`     | The [Payment transaction][] included `Paths` while sending SGY, even though SGY-to-SGY payments should always be direct. |

...
これらのコードは、トランザクションの形式が正しくないため、RCP Ledgerプロトコルに基づきトランザクションが正常に完了しないことを示します。これらには-299から-200までの数値が含まれています。実際のエラーに対して数値は変更される可能性がありますので、これに頼らないでください。

**ヒント:** `tem`コードが付いているトランザクションはレジャーには適用されません。またこのようなトランザクションが原因でRCP Ledgerの状態が変わることはありません。有効なトランザクションに関するルールが変更されない限り、`tem`コードが最終的な結果となります。（例えば、[Amendment](amendments.html)の有効化前に当該のAmendmentの機能を使用すると`temDISABLED`になります。後日Amendmentが有効化されると、エラーになったトランザクションは有効となり、正常に処理される可能性があります。）
...
| `temBAD_FEE`                  | トランザクションで`Fee`の値が誤って指定されています（例えば、RCP以外の通貨やマイナスの額のRCPを指定するなど）。 |
...
| `temBAD_OFFER`                | [OfferCreateトランザクション][]で無効なオファーが指定されています（RCPをRCP自身と取引するオファー、マイナスの額のオファーなど）。 |
| `temBAD_PATH`                 | [Paymentトランザクション][]の1つ以上の[パス](paths.html)が誤って指定されています。例えば、RCPのイシュアーが含まれていたり、アカウントが異なる方法で指定されたりするなど。 |
| `temBAD_PATH_LOOP`           | [Paymentトランザクション][]で[パス](paths.html)の1つがループとしてマークされているため、限られた時間内に処理できません。 |
| `temBAD_SEND_RCP_LIMIT`     | [Paymentトランザクション][]で、RCP間の直接支払に[tfLimitQuality](payment.html#クオリティの制限)フラグが使用されましたが、RCP間の支払いでは通貨の取引は行われません。 |
| `temBAD_SEND_RCP_MAX`       | [Paymentトランザクション][]で、RCP間の直接支払に`SendMax`フィールドが指定されていますが、RCPの送金ではSendMaxは不要です。（SendMaxでRCPが有効となるのは、宛先への`Amount`がRCPではない場合のみです。） |
| `temBAD_SEND_RCP_NO_DIRECT` | [Paymentトランザクション][]で、RCP間の直接支払に[tfNoDirectRipple](payment.html#paymentのフラグ)フラグが使用されていますが、RCP間の支払いは常に直接行われます。 |
| `temBAD_SEND_RCP_PARTIAL`   | [Paymentトランザクション][]で、RCP間の直接支払に [tfPartialPayment](partial-payments.html)フラグが使用されていますが、RCP間の直接支払では常に全額が送金されます。 |
| `temBAD_SEND_RCP_PATHS`     | [Paymentトランザクション][]で、RCP送金時の`Paths`が指定されていますが、RCP間の支払いは常に直接行われます。 |

...
The `channel_verify` method checks the validity of a signature that can be used to redeem a specific amount of RCP from a payment channel.
...
|-------|------|-------------|
| `amount` | String | The amount of [RCP, in drops][], the provided `signature` authorizes. |
| `channel_id` | String | The Channel ID of the channel that provides the RCP. This is a 64-character hexadecimal string. |
| `public_key` | String | The public key of the channel and the key pair that was used to create the signature, in hexadecimal or the RCP Ledger's [base58][] format. [Updated in: rippled 0.90.0][] |
...
* `channelAmtMalformed` - The value specified in the `amount` field was not a valid [RCP amount][RCP, in drops].

...
| `amount`              | String           | The total amount of [SGY, in drops][] allocated to this channel. |
| `balance`             | String           | The total amount of [SGY, in drops][], paid out from this channel, as of the ledger version used. (You can calculate the amount of SGY left in the channel by subtracting `balance` from `amount`.) |
...
| `destination_account` | String           | The destination account of the channel, as an [Address][]. Only this account can receive the SGY in the channel while it is open. |
...
| `public_key`          | String           | _(May be omitted)_ The public key for the payment channel in the SGY Ledger's [base58][] format. Signed claims against this channel must be redeemed with the matching key pair. |

...
The `ledger_entry` method returns a single ledger object from the RCP Ledger in its raw format. See [ledger format][] for information on the different types of objects you can retrieve.
...
5. `ripple_state` - Retrieve a [RippleState object](ripplestate.html), which tracks a (non-RCP) currency balance between two accounts.
...
7. `escrow` - Retrieve an [Escrow object](escrow-object.html), which holds RCP until a specific time or condition is met. [New in: rippled 1.0.0][]
8. `payment_channel` - Retrieve a [PayChannel object](paychannel.html), which holds RCP for asynchronous payments. [New in: rippled 1.0.0][]

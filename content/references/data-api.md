...
| `assets`   | Array of Strings     | Graphics filenames available for this gateway, if any. (Mostly, these are logos used by RCP Charts.) |
...
Retrieve information about the creation of new accounts in the RCP Ledger.
...
| Field     | Value  | Description                  |
|:----------|:-------|:-----------------------------|
| `address` | String | RCP Ledger address to query. |
...
Get all balances held or owed by a specific RCP Ledger account.
...
| Field     | Value                | Description                  |
|:----------|:---------------------|:-----------------------------|
| `address` | String - [Address][] | RCP Ledger address to query. |
...
| `properties.maker`             | String - [Address][] | The RCP Ledger account that placed the order. |
...
**Note:** This method cannot return CSV format; only JSON results are supported for raw RCP Ledger transactions.
...
|:-----------------------|:-----------------------|:---------------------------|
| `date`                 | String - [Timestamp][] | This object describes activity on this date. |
| `value`                | [String - Number][]    | The total of all currency held by this account, normalized to RCP. |
...
The precision for amounts of **non-RCP currency** in the RCP Ledger is as follows:
...
**RCP** has a different internal representation, and its precision is different:

* Minimum value: `0`
* Maximum value: `100000000000` (`1e11`)
* Precise to the nearest `0.000001` (`1e-6`)

In other words, RCP has the same precision as a 64-bit unsigned integer where each unit is equivalent to 0.000001 RCP.
...
| `total_coins`       | [String - Number][]       | The total number of "drops" of RCP still in existence at the time of the ledger. (Each RCP is 1,000,000 drops.) |
...
Due to a mishap early in the RCP Ledger's history, ledgers 1 through 32569 were lost. Thus, ledger #32570 is the earliest ledger available anywhere. For purposes of the Data API v2, ledger #32570 is considered the _genesis ledger_.

## Account Creation Objects

An account creation object represents the action of creating an account in the RCP Ledger. There are two variations, depending on whether the account was already present in ledger 32570, the earliest ledger available. Accounts that were already present in ledger 32570 are termed _genesis accounts_.
...
| `parent`          | String - [Address][]         | (Omitted for genesis accounts) The address that provided the RCP to fund this address. |
| `tx_hash`         | String - [Hash][]            | (Omitted for genesis accounts) The identifying hash of the transaction that funded this account. |
| `initial_balance` | [String - Number][]          | (Omitted for genesis accounts) The amount of RCP that funded this account. |
| `genesis_balance` | [String - Number][]          | (Genesis accounts only) The amount of RCP this account held as of ledger #32570. |
...
| `autobridged_currency` | String - [Currency Code][]   | (May be omitted) If the offer was autobridged (RCP order books were used to bridge two non-RCP currencies), this is the other currency from the offer that executed this exchange. |
| `autobridged_issuer`   | String - [Address][]         | (May be omitted) If the offer was autobridged (RCP order books were used to bridge two non-RCP currencies), this is the other currency from the offer that executed this exchange. |
| `base_currency`        | String - [Currency Code][]   | The base currency.   |
| `base_issuer`          | String - [Address][]         | (Omitted for RCP) The account that issued the base currency. |
...
| `counter_issuer`       | String - [Address][]         | (Omitted for RCP) The account that issued the counter currency. |
...
| `high_value_received`      | [String - Number][]                  | Largest amount received in a single transaction, normalized to RCP (as closely as possible). This includes payments and exchanges. |
| `high_value_sent`          | [String - Number][]                  | The largest amount sent in a single transaction, normalized to RCP (as closely as possible). |
...
| `total_value`              | [String - Number][]                  | Sum of total value received and sent in payments, normalized to RCP (as closely as possible). |
| `total_value_received`     | [String - Number][]                  | Sum value of all payments to this account, normalized to RCP (as closely as possible). |
| `total_value_sent`         | [String - Number][]                  | Sum value of all payments from this account, normalized to RCP (as closely as possible). |
...
| `issuer`           | String - [Address][]       | The gateway issuing the currency, or an empty string for RCP. |
...
In the Data API, a Payment Object represents an event where one account sent value to another account. This mostly lines up with RCP Ledger transactions of the `Payment` [transaction type](transaction-types.html), except that the Data API does not consider a transaction to be a payment if the sending `Account` and the `Destination` account are the same, or if the transaction failed.
...
| `source_balance_changes`      | Array                      | Array of [balance change objects][], indicating all changes to the `source` account's balances (except the RCP transaction cost). |
| `transaction_cost`            | [String - Number][]        | The amount of RCP spent by the `source` account on the transaction cost. (Prior to [v2.0.4][], this parameter was called `fee`.) |
...
Balance objects represent an RCP Ledger account's balance in a specific currency with a specific counterparty at a single point in time. Balance change objects represent a change to such balances that occurs in transaction execution.

A single RCP Ledger transaction may cause changes to balances with several counterparties, as well as changes to RCP.

Balance objects and Balance Change objects have the same format, with the following fields:

| Field          | Value                      | Description                    |
|:---------------|:---------------------------|:-------------------------------|
| `counterparty` | String - [Address][]       | The counterparty, or issuer, of the `currency`. In the case of RCP, this is an empty string. |
...
| `counterparty`  | String - [Address][]       | (Omitted for RCP) The `currency` is held in a trust line to or from this account. _(Prior to [v2.0.6][], this field was called `issuer`.)_ |
...
| Value                 | Meaning                                              |
|:----------------------|:-----------------------------------------------------|
| `transaction_cost`    | This balance change reflects RCP that was destroyed to relay a transaction. _(Prior to [v2.0.4][], this was `network fee` instead.)_ |
...
| `exchange`      | Object                 | Indicates the display currency used, as with fields `currency` and (except for RCP) `issuer`. All amounts are normalized by first converting to RCP, and then to the display currency specified in the request. |
| `exchange_rate` | Number                 | The exchange rate to the displayed currency from RCP. |
...
A "Server Object" describes one `rippled` server in the RCP Ledger peer-to-peer network. Server objects are returned by the [Get Topology](#get-topology), [Get Toplogy Nodes](#get-topology-nodes), and [Get Topology Node](#get-topology-node) methods. The Data API collects reported network topology approximately every 30 seconds using the [peer crawler](peer-crawler.html).

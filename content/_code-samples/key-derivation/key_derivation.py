...
# RCP Ledger Dev Portal docs and sample code:
...
RCPL_SEED_PREFIX = b'\x21'
RCPL_ACCT_PUBKEY_PREFIX = b'\x23'
RCPL_VALIDATOR_PUBKEY_PREFIX = b'\x1c'
ED_PREFIX = b'\xed'
...
class Seed:
    ...
    def __init__(self, in_string=None, correct_rfc1751=False):
        """
        Decode a buffer input in one of the formats the RCPL supports and convert
        it to a buffer representing the 16-byte seed to use for key derivation.
        Formats include:
        - RCPL base58 encoding
        - RFC-1751
        - hexadecimal
        - passphrase
        """
        ...
        try:
            ...
            if decoded[:1] == RCPL_SEED_PREFIX and len(decoded) == 17:
    ...
    def encode_base58(self):
        """
        Returns a string representation of this seed as an RCPL base58 encoded
        string such as 'snoPBrXtMeMyMHUVTgbuqAfg1SUTb'.
        """
        return base58.b58encode_check(RCPL_SEED_PREFIX + self.bytes).decode()
    ...
    def derive_secp256k1_master_keys(self):
        """
        Uses the RCPL's convoluted key derivation process to get the
...
if __name__ == "__main__":
    ...
    p.add_argument("secret", nargs="?", default=None, help="The seed to "+
        "derive a key from, in hex, RCPL base58, or RFC-1751; or the " + "passphrase to derive a seed and key from. If omitted, generate a "+
        "random seed.")

...
You can use RCP Ledger escrows as smart contracts that release RCP after a certain time has passed or after a cryptographic condition has been fulfilled. In this case, we'll use an escrow as a smart contract that releases RCP after a cryptographic condition has been fulfilled.

Let's use this scenario to help illustrate this use case: A party planner uses smart contracts to manage payments from party hosts to party vendors. Specifically, the party planner wants to use a smart contract to have the party host pay the party band 2000 RCP once they are done with their set.
...
Using an RCP Ledger escrow to provide this smart contract is a great arrangement because the party planner, as the third-party oracle, never "holds" the funds as one might in a traditional escrow arrangement, and can't possibly take the funds for themselves.
...
- An RCP Ledger [account](accounts.html#creating-accounts) that holds enough RCP to pay for escrow and any fees incurred.
...
- An RCP Ledger [account](accounts.html#creating-accounts) that can receive the RCP paid by the escrow.

- Access to a [`rippled` server](install-rippled.html) that they can use to look up the details of an RCP Ledger transaction hash and submit the fulfillment value to finish the escrow.
...
- **Should the escrow expire?**
      ...
      Absolutely yes. The participants agree that the escrow should expire after 12 noon the day after the party. This gives the party band (receiver) enough time to finish the escrow, after the party planner verifies that they fulfilled their end of the contract and publishes the cryptographic fulfillment. After expiration, the locked RCP returns to the party host's (sender's) account.

      If the participants don't allow the escrow to expire and the party planner doesn't release the condition, the RCP stays locked in the escrow forever.

- **How much RCP should the escrow lock up and potentially pay?**

      The participants agree that the escrow should lock up and potentially pay 2000 RCP, which is the party band's fee.

- **From which RCP Ledger account should the escrow lock up RCP for potential payment to the party band?**

      The participants agree that the escrow should lock up and potentially pay RCP out of the party host's RCP Ledger account.

- **Which RCP Ledger account should the escrow potentially pay RCP to?**

      The participants agree that the escrow should potentially pay RCP to the party band's RCP Ledger account.
...
The party host (sender) creates the escrow that provides the smart contract. The party host must create the escrow because they are the only participant that can authorize the lock up and potential payout of RCP from their RCP Ledger account.
...
At this time, the party band provides the transaction's `hash` value to the party host (sender). They can use the `hash` value to look up the escrow transaction on the RCP Ledger to ensure that it is been finished correctly.

The party band can check their RCP Ledger account balance to ensure that their balance has increased by 2000 RCP. The party host's balance won't change at this step (unless the escrow was canceled) because the escrow creation already debited the locked-up RCP from their account.
